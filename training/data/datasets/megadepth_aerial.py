...
class MegaDepthAerialDataset(BaseDataset):
    ...
    #         ROOT: Root directory of preprocessed data
    #         split_file: NPZ file containing pairs and metadata
    #         segmentation_root: Optional path to segmentation masks for sky removal
    #         min_num_images: Minimum number of images per sequence
    #         len_train/len_test: Dataset length for training/testing
    #         max_depth: Maximum depth value (meters) - closer threshold for aerial
    #         depth_percentile: Percentile for depth outlier removal
    #         use_pairs: Whether to use predefined pairs or sample randomly
    #         expand_ratio: Ratio for expanding nearby frames
    #         remove_sky: Whether to remove sky using segmentation
    #     """
    #     super().__init__(common_conf=common_conf)
    ...
    #     # For your current setup with scenes 0000 and 0001
    #     # Modify this if you add more scenes later
    #     if split == 'train':
    #         # Use both scenes for training since you only have 2
    #         self.valid_scenes = ['0000', '0001']
    #     elif split in ['val', 'test']:
    #         # You might want to use one scene for validation
    #         # Or skip validation for now with limited data
    #         self.valid_scenes = ['0001']  # Or empty list if no validation
    ...
    #         # Find all images belonging to this scene
    #         if self.images_scene_name is not None:
    #             for img_id, img in enumerate(self.images):
    #                 if img is not None and self.images_scene_name[img_id] == scene:
    #                     scene_images.append((img_id, str(img)))
    #         else:
    #             # Fallback: check which images exist in the scene directory
    #             scene_path = osp.join(self.ROOT, scene_str)
    #             if osp.exists(scene_path):
    #                 for img_id, img in enumerate(self.images):
    #                     if img is not None:
    #                         img_path = osp.join(scene_path, str(img) + '.jpg')
    #                         if osp.exists(img_path):
    #                             scene_images.append((img_id, str(img)))
            
    #         self.scene_to_images[scene_str] = scene_images
            
    # def _filter_to_valid_scenes(self):
    #     """Filter pairs to only include valid scenes."""
    ...
    #         if pair['scene_id'] in valid_scene_ids:
    ...
    def __len__(self):
        # For training, use the specified length
        # This allows multiple epochs through the same data
        if self.training:
            return self.len_train
        else:
            # For validation, use actual number of pairs
            return min(len(self.pairs), self.len_train)
    ...
    #         scene = str(self.scenes[scene_id])
    ...
    #             img_data = self._load_image_data(scene, img_name)
    #             if img_data is None:
    #                 continue
    ...
    #                 original_size,
    #                 target_image_shape,
    #                 filepath=osp.join(scene, img_name),
    #             )
    ...
    #     try:
    #         # The img_name from NPZ already includes the full filename (e.g., "0001_600.jpeg.jpg")
    #         img_path = osp.join(scene_path, img_name)
    ...
    #         # For depth and npz files, we need to replace .jpg with the appropriate extension
    #         # The pattern is: xxxxx.jpeg.jpg -> xxxxx.jpeg.exr and xxxxx.jpeg.npz
    #         if img_name.endswith('.jpg'):
    #             base_name = img_name[:-4]  # Remove '.jpg'
    #         else:
    #             # Fallback for any unusual cases
    #             base_name = img_name
                
    #         # Load depth map (OpenEXR format)
    #         depth_path = osp.join(scene_path, base_name + '.exr')
    #         if not osp.exists(depth_path):
    #             logging.warning(f"Depth not found: {depth_path}")
    #             return None
    ...
    #         # Load camera parameters
    #         npz_path = osp.join(scene_path, base_name + '.npz')
    #         if not osp.exists(npz_path):
    #             logging.warning(f"Camera params not found: {npz_path}")
    #             return None
    ...
    #         # Optional: Load and apply segmentation mask for sky removal
    #         if self.remove_sky and self.segmentation_root:
    #             seg_path = osp.join(self.segmentation_root, scene, base_name + '.png')
    #             if osp.exists(seg_path):
    #                 segmap = cv2.imread(seg_path, cv2.IMREAD_GRAYSCALE)
    #                 # Remove sky (ADE20k label 2 is sky)
    #                 depth_map[segmap == 2] = 0
