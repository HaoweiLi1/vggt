...
def main():
    ...
    # Load split information from npz files
    def load_split_scenes(data_root, split_name):
        """Load scene names from train.npz or val.npz files."""
        if split_name == 'train':
            split_file = data_root / 'train.npz'
        elif split_name == 'test' or split_name == 'val':
            split_file = data_root / 'val.npz'
        else:
            print(f"Warning: Unknown split '{split_name}', using empty scene list")
            return []
        
        if split_file.exists():
            try:
                split_data = np.load(split_file, allow_pickle=True)
                if 'scenes' in split_data:
                    scenes = split_data['scenes'].tolist()
                    print(f"Loaded {len(scenes)} scenes from {split_file.name}: {scenes}")
                    return scenes
                else:
                    print(f"Warning: 'scenes' key not found in {split_file.name}")
                    return []
            except Exception as e:
                print(f"Warning: Failed to load {split_file.name}: {e}")
                return []
        else:
            print(f"Warning: Split file {split_file} not found")
            # Fallback to hardcoded mapping
            fallback_mapping = {
                'train': ['0000', '0001', '0002', '0003'],
                'test': ['0015'],
                'val': ['0015']
            }
            scenes = fallback_mapping.get(split_name, [])
            print(f"Using fallback scene list for '{split_name}': {scenes}")
            return scenes
